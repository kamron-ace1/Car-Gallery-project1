  ...
  card.innerHTML = `
        <div class="relative overflow-hidden aspect-[16/10] bg-zinc-900 border border-white/5 mb-4 rounded-3xl">
            ...
            <div class="absolute top-4 right-4 flex flex-col gap-2">
                ...
                <!-- Wishlist Button -->
                <button onclick="event.stopPropagation(); toggleWishlist(event, ${car.id})" 
                        class="w-9 h-9 rounded-full bg-black/60 backdrop-blur-sm border border-white/10 flex items-center justify-center
                            hover:bg-[#1c69d4] hover:border-[#1c69d4] hover:scale-110 transition-all duration-300 wishlist-btn">
...
document.getElementById("newsletterForm")?.addEventListener("submit", (e) => {
  e.preventDefault();
  showToast(
    translations[currentLanguage]?.["thank-you"] ||
...
document.addEventListener("click", (e) => {
  ...
  if (e.target === document.querySelector("#authModal > div:first-child"))
    closeAuth();
  if (e.target === document.querySelector("#wishlistModal > div:first-child"))
    closeWishlist();
  if (e.target === document.querySelector("#searchModal > div:first-child"))
    closeSearch();
});
